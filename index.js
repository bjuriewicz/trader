...
const firstDealLimitPrice = Number(process.argv(2));
console.log('firstDealPrice: ' + firstDealLimitPrice);
const market = 'btc-nxs';
const sourceCoin = 'BTC';
const targetCoin = 'NXS';
const diffBuy = 0.00001;
const diffSell = 0.00001;
const priceOffsetFactor = 0.005;

let buy = process.argv(3) === '1';
console.log('buy: ' + buy);
...
async function trade() {
   ...
   if (buy) {  
       if ((previousDealPrice == null && price <= firstDealLimitPrice)
           ...
           let res = await buyLimit(market, quantity, price);
           if (res) {
   ...
   //sell
       ...
       if ((previousDealPrice == null && price >= firstDealLimitPrice)
           ...
           //price = price - (price * priceOffsetFactor);
           let res = await sellLimit(market, availableTarget, price);
           if (res) {
               previousDealPrice = price;
               buy = true;
               let d = new Date().toLocaleString();
